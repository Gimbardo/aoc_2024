...
fn problem1(contents: String) -> u32 {
  ...
  count_xmas(formatted_content, directions_hash)
}
...
fn problem2(contents: String) -> u32 {
  ...
  count_x_shapes(formatted_content.clone(), m_indexes)
}
    ...
    for direction_coords in directions_maps.iter() {
      ...
      if contents[m_y_index as usize][m_x_index as usize] == "M".chars().next().unwrap() {
        result.push(HashMap::from([
          ...
          ("direction_x".into(), { *direction_coords.get("x").unwrap() }),
          ("direction_y".into(), { *direction_coords.get("y").unwrap() })        
        ]));
      }
    }
  }
...
fn count_x_shapes(contents: Vec<Vec<char>>, m_indexes: Vec<HashMap<String, usize>>) -> u32 {
    ...
    for m_index in m_indexes.iter() {
      ...
      let x_shape_values = [contents[(m_y + 1) as usize][(m_x + 1) as usize],
