...
fn problem1(contents: String) -> u32 {
  ...
  let mut formatted_content: Vec<Vec<u32>> = surround_content(contents);
...
fn surround_content(contents: String) -> Vec<Vec<u32>> {
  ...
  for line in contents.lines() {
    final_content.push(format!("{line}").chars().map(|c| if let Some(value) = c.to_digit(10) { return value } else { return 100; } ).collect());
  }
